...
import { getEvalMatrix, runEvaluation, getRecommendations, saveTender, runAnomalyDetection, getAnomalyFlags } from '../services/evaluationApi';
...
const Dashboard = () => {
  ...
  // Evaluation, Anomaly & Recommendation state
  ...
  // Selected tender for matrix (org role picks from their list)
  const [activeTenderId, setActiveTenderId] = useState(null);

  // Load recommendations on mount (bidder role)
  useEffect(() => {
    if (user.role === 'bidder') {
      setRecsLoading(true);
      getRecommendations()
  ...
  // Load matrix + anomalies when org selects a tender
  useEffect(() => {
    if (!activeTenderId) return;
    setEvalLoading(true);
    Promise.all([
      getEvalMatrix(activeTenderId),
      getAnomalyFlags(activeTenderId)
    ])
      .then(([evalRes, anomalyRes]) => {
        setEvalRows(evalRes.data.data);
  ...
  const handleRunEvaluation = async () => {
    if (!activeTenderId) return;
    setEvalLoading(true);
    await runEvaluation(activeTenderId);
  ...
  const handleSaveTender = async (tenderId) => {
    ...
    setSavedTenders(prev => [...prev, tenderId]);
  };

  const scoreColor = (v) =>
    v >= 8.5 ? 'bg-green-100 text-green-800' :
    v >= 7.0 ? 'bg-yellow-100 text-yellow-800' :
...
<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-white rounded-lg shadow-md p-6 text-center">
                <div className="text-4xl mb-2"> </div>
                <h5 className="text-xl font-semibold mb-1">12</h5>
                <p className="text-gray-500">Received Bids </p>

              </div>
              <div className="bg-white rounded-lg shadow-md p-6 text-center">
                <div className="text-4xl mb-2"> </div>
                <h5 className="text-xl font-semibold mb-1">18</h5>
  ...
  <div className="border-b border-gray-200 p-6">
    <div className="flex justify-between items-center">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {evalLoading ? 'Scoring...' : '▶ Run Evaluation'}
        </button>
      </div>
    </div>
  </div>

  {/* Anomaly Flags Banner */}
  {anomalyFlags.length > 0 && (
    <div className="mx-6 mt-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-center gap-2">
      <AlertCircle className="w-4 h-4 text-red-600 shrink-0" />
      <span className="text-sm text-red-800 font-medium">
        {anomalyFlags.filter(f => f.status === 'open').length} anomaly flag(s) detected on this tender.
      </span>
      <div className="flex gap-2 ml-2 flex-wrap">
        {anomalyFlags.slice(0, 3).map(flag => (
          <span key={flag.id} className={`px-2 py-0.5 rounded-full text-xs font-medium
  ...
  <div className="p-6">
    {!activeTenderId ? (
      <p className="text-gray-400 text-sm text-center py-6">
        Select a tender above to view or run evaluation.
      </p>
    ) : evalLoading ? (
      ...
      <p className="text-gray-400 text-sm text-center py-6">
        No evaluations yet. Click "Run Evaluation" to score all bids.
      </p>
    ) : (
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              {['Rank', 'Bidder', 'Price', 'Quality', 'Timeline', 'Experience', 'Total Score', 'Status'].map(h => (
          ...
          <tbody className="divide-y divide-gray-200">
            {evalRows.map(row => (
              <tr key={row.id} className={`hover:bg-gray-50 ${row.rank === 1 ? 'bg-green-50' : ''}`}>
                <td className="px-4 py-3 font-bold">
                  {row.rank === 1 ? '🥇' : row.rank === 2 ? '🥈' : row.rank === 3 ? '🥉' : `#${row.rank}`}
                </td>
                <td className="px-4 py-3 font-semibold text-gray-900">
                  {row.Bidder?.company || row.Bidder?.name}
                  {!row.passesMandatoryChecks && (
                    <span className="ml-2 px-1 py-0.5 bg-red-100 text-red-700 rounded text-xs">Disqualified</span>
                  )}
                </td>
                {[row.priceScore, row.qualityScore, row.timelineScore, row.experienceScore].map((s, i) => (
                  <td key={i} className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs ${scoreColor(s)}`}>{s}</span>
                  </td>
                ))}
                <td className="px-4 py-3 font-bold text-gray-900">{row.totalScore}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium
                    ${row.status === 'awarded'    ? 'bg-green-100 text-green-800' :
                      row.status === 'rejected'   ? 'bg-red-100 text-red-800' :
                      row.status === 'evaluated'  ? 'bg-blue-100 text-blue-800' :
...
<div className="bg-white rounded-lg shadow-md">
  ...
  <div className="p-6">
    {recsLoading ? (
      ...
      <p className="text-gray-400 text-sm text-center py-6">
        No recommendations yet. Complete your bidder profile to get matches.
      </p>
    ) : (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {recommendations.map(rec => {
          const daysLeft = Math.floor((new Date(rec.Tender?.deadline) - new Date()) / 86400000);
          const isSaved  = savedTenders.includes(rec.tenderId);
          return (
            <div key={rec.tenderId} className="border-2 border-green-200 bg-white rounded-lg shadow-sm">
              <div className="p-4">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button
                    onClick={() => handleSaveTender(rec.tenderId)}
                    className={`${isSaved ? 'text-red-500' : 'text-gray-300'} hover:text-red-500 transition-colors`}
                  >
                    <Heart className="w-5 h-5" fill={isSaved ? 'currentColor' : 'none'} />
                  </button>
                </div>
                <h6 className="font-semibold text-gray-900 mb-2">{rec.Tender?.title}</h6>
                <p className="text-gray-500 text-sm mb-3">
                  ${rec.Tender?.budget?.toLocaleString()} • {daysLeft} days left
                </p>
